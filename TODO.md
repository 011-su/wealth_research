...
- [x] **Step 2.C (Destatis age-rising mortality) implemented 2026-06-22**
  (`mortality_step2.m`, `data/derived/mortality_hazard.csv`,
  `data/make_mortality_hazard.py`; toggle `params.mortality='step2'`). Fixes
  the constant-hazard artifacts: mean age at death 57.9→80.9 (German LE),
  aging-out @h_max 19.7%→1.8%. **This resolves the degenerate BWR
  calibration** — bwr is now responsive to theta_b (was ≈lambda_bar for any
  theta_b). See `compare_mortality.m`.
- [ ] **Re-calibrate theta_b UNDER step2 mortality (now meaningful).** At
  theta_b=1 the realistic mortality gives bwr=0.0092 (mean bequest €278k,
  aggregate ~€181 bn/yr) — LOWER than step1, because agents now die old
  (~81) after decumulating through retirement, so they die poorer than peak.
  bwr now RISES with theta_b (stronger motive → less decumulation → wealthier
  deaths), so calibrating theta_b up to the data target (~0.03 / ~€300 bn)
  is well-identified. Run `calibrate_step1.m` with `params.mortality='step2'`.
  DECIDE whether step2 becomes the default mortality (currently step1 is, to
  keep the validation baseline + tests unchanged).
